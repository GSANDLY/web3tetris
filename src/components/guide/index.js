...
export default class Guide extends React.Component {
  ...
  render() {
    states.addNetworkToMetaMask();
    states.readScoreList();
    if (this.state.isMobile) {
      return (
        null
      );
    ...
    return (
      <div style={{ display: this.state.isMobile ? 'none' : 'block' }}>
        <p style={{ color: 'white', position: 'absolute', right: '-32em', top: '1em' }}>
          排行榜
          <ul style={{ marginTop: '1em', listStyle: 'auto' }}>
            {
              states.scoreList.map(item => (<li style={{ paddingTop: '10px' }}>{ item }</li>))
            }
          </ul>
        </p>
